<template>
    <div class="p-6 bg-gray-50 rounded-lg shadow-lg max-w-7xl mx-auto">
        ...
        <table class="table-auto text-center border-collapse">
          <thead class="bg-gray-100">
            <tr>
              ...
              <th class="px-4 py-3 text-sm font-medium text-gray-700">All</th>
              <th v-for="week in weeks" :key="week" class="px-4 py-3 text-sm font-medium text-gray-700">
                {{ week }}th
              </th>
            </tr>
          </thead>
              ...
              <td v-for="(week, weekIndex) in weeks" :key="weekIndex" class="px-4 py-3">
                <div class="flex items-center space-x-2 justify-center">
                  ...
                  <div v-if="halfWorkingWeekend" class="ml-2">
                    <select v-model="day.weeks[weekIndex].dayType" class="border rounded-md">
      ...
      <div class="flex justify-end mt-6 space-x-4">
        <button class="bg-blue-500 text-white px-6 py-2 rounded-md hover:bg-blue-600 shadow text-sm">
          Save
        </button>
        <button class="bg-gray-300 text-gray-700 px-6 py-2 rounded-md hover:bg-gray-400 shadow text-sm">
          Cancel
        </button>
      </div>
    </div>
